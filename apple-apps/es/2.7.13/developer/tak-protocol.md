...
|---------|---------|----------|
| `.atakPlugin` (72) | `handleATAKPluginPacket(_:)` | Decodificar `TAKPacket` protobuf desnudo; convertir PLI / GeoChat a `CoTMessage`; reenviar a los clientes TAK a través de `TAKServerManager.shared.broadcast(_:)`. |
| `.atakPluginV2` (78) | `handleATAKPluginV2Packet(_:)` | Zstd-decomprimir con `TakCompressor`; reconstruir CoT XML con `CotXmlBuilder`; eliminar el prólogo XML y el espacio en blanco entre etiquetas; reenviar XML sin procesar a través de `broadcastRawXml(_:)` para que el detalle de la forma (`<link point>` vértices, colores, trazo) sobreviva. Route CoT (`b-m-r`) desencadena el efecto secundario [paquete de datos de ruta](#route-data-packages). |
| `.atakForwarder` (257) | `manejaATAKForwarderPacket(_:)` | Entrega a `GenericCoTHandler.handleIncomingForwarderPacket(_:)`, que vuelve a ensamblar fragmentos de Fountain y zlib-descomprime el CoT XML resultante antes de transmitir. |

## TAKPacket-SDK

El paquete Swift [TAKPacket-SDK](https://github.com/meshtastic/TAKPacket-SDK) está anclado en`Meshtastic.xcworkspace/.../Package.resolved`. El puente utiliza tres API:
...
Consulte también [Capa de transporte](transport.html) para el mapa de extensión de AccessoryManager.
