...
| `Extensiones/Logger.swift` | Logadores OSLog mecanografiados para todos los subsistemas |
| `Persistencia/PersistenceController.swift` | Configuración de SwiftData `ModelContainer` |
| `Modelo/MeshtasticSchema.swift` | `Esquema versionado` + `Plan de migración de esquema` |
...
Las vistas que necesitan conectividad inyectan`@EnvironmentObject var accessoryManager: AccessoryManager`. Vistas que necesitan navegación inyecta`@EnvironmentObject var router: Router`.
