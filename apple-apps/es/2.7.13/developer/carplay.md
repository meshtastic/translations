...
|---|---|---|
| `CarPlaySceneDelegate` | `Meshtastic/CarPlay/CarPlaySceneDelegate.swift` | `CPTemplateApplicationSceneDelegate` que construye y gestiona la interfaz de usuario de dos pestañas |
| `CarPlayIntentDonation` | `Meshtastic/CarPlay/CarPlayIntentDonation.swift` | Dona interacciones `INSendMessageIntent` entrantes y salientes para que las conversaciones aparezcan en los mensajes de CarPlay y Siri pueda leerlas en voz alta |
| `EnviarMensajeIntenciónHandler` | `Meshtastic/Intenciones/SendMessageIntentHandler.swift` | Maneja `INSendMessageIntent` - resuelve destinatarios/canales y envía el mensaje a través del transporte activo |
| `SearchForMessagesIntentHandler` | `Meshtastic/Intenciones/SearchForMessagesIntentHandler.swift` | Maneja `INSearchForMessagesIntent` |
| `SetMessageAttributeIntentHandler` | `Meshtastic/Intenciones/SetMessageAttributeIntentHandler.swift` | Maneja `INSetMessageAttributeIntent` (marcar como leído) |
| `IntentHandler` | `Meshtastic/Intenciones/IntentHandler.swift` | Enruta `INIntent`s al manejador apropiado |
