...
Die Meshtastic-App zeigt automatisch Temperaturen, Entfernungen, Geschwindigkeiten und Zeiten in den Geräten an, für die die Verwendung deines Geräts konfiguriert ist - keine Einstellungen, die innerhalb der App geändert werden.

---

## Wie Es Funktioniert

Meshtastic-Funkgeräte übertragen Daten immer in **metrischen Einheiten** (Meter, °C, km/h, hPa usw.). Wenn die App diese Daten erhält, übergibt sie sie an das integrierte Formatierungssystem deines Geräts, das Werte in jedem von dir gewählten Einheitensystem unter **Einstellungen → Allgemein → Sprache & Region** konvertiert und anzeigt.
...
> **Tip — du müssen niemals Einheiten innerhalb der App umschalten. ** Ändern du deine Systemmesseinstellungen und jeder Bildschirm in Meshtastic wird automatisch aktualisiert - Knotendetails, Telemetriediagramme, Wetter, Höhe und mehr.

## Temperatur

Temperaturwerte von Umgebungssensoren und Wettervorhersagen werden als **°C** übertragen und je nach Temperatureinheit deines Geräts entweder als **°C** oder **°F** angezeigt.

| deine Einstellung | du sehen |
|---|---|
| Celsius | 22 °C |
| Fahrenheit | 72 °F |
...
| deine Einstellung | Kleine Entfernung | Große Entfernung | Höhe |
|---|---|---|---|
...
- **Knotendetail** — Höhe, Entfernung von deiner Position
- **Karte** — Wegpunktentfernungen, Spurrouten-Hüfenentfernungen
- **Kompass** — Abstand zum ausgewählten Knoten
- **Höhendiagramm** - Y-Achsen-Beschriftungen passen sich deinem Gebietsschema an

## Geschwindigkeit

Die GPS-Bodengeschwindigkeit wird in der bevorzugten Geschwindigkeitseinheit deines Standorts angezeigt.
...
Die Geschwindigkeit wird auf dem Bildschirm **GPS-Status** angezeigt, wenn dein Gerät über einen aktiven GPS-Fix verfügt.
...
Einige Einheiten sind internationale Standards und werden unabhängig von deinem Gebietsschema auf die gleiche Weise angezeigt:
...
Alle Zeitstempel in der gesamten App - zuletzt gehört, Nachrichtenzeiten, Telemetrieprotokolle, Diagrammachsen - folgen den Datums- und Uhrzeiteinstellungen deines Geräts.
...
Die App verwendet auch **relative Zeit**, wenn es sinnvoll ist - zum Beispiel "vor 5 Minuten" oder "vor 2 Stunden" in der Knotenliste - die automatisch in der Sprache deines Geräts lokalisiert wird.

## Ändern du dein Messsystem

dein Messsystem (metrisch vs. imperial) ist an deine Regionseinstellung gebunden. Um es zu ändern, ohne deine Sprache zu ändern:

1. Öffnen **Einstellungen → Allgemein → Sprache & Region**
2. Tippe auf **Messsystem**
3. Wähle **Metrisch**, **US** oder **UK**
