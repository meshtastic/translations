...
* **RSSI (Received Signal Strength Indicator):** Dies ist die **Lautstärke** der Rohleistung, die auf Ihre Antenne trifft.
...
> * **SNR** ist, wie leicht Sie die Stimme Ihres Freundes von der Hintergrundgeräusche unterscheiden können.
...
| Schlecht | 1 | RSSI fällt auf`-120 dBm`Oder schlimmer, **ODER** SNR innerhalb`5.5 dB`Des absoluten Bruchpunkts Ihrer Voreinstellung | Kaum hängend - am Rande der Reichweite oder starke Störungen. |
| Keiner | 0 | RSSI schlimmer als`-126 dBm`**UND** SNR ist gesunken`7.5 dB`Unterhalb der idealen Grenze | Übertragung vollständig in statischer Vergrabung. |
...
> **Tip — Keine Panik wegen des niedrigen RSSI:**Sie könnten einen scheinbar schrecklichen RSSI-Wert wie`-118 dBm`. Auf einem Handy hätten Sie null Balken. Aber wenn Sie eine SNR von`+2 dB`, Meshtastic wird immer noch ein starkes Signal zeigen! *Die Bibliothek ist ruhig, so dass das Flüstern perfekt zu hören ist. *

> **Warning — Achten Sie auf lokale Geräusche:**Wenn Sie eine massive Antenne anschließen und einen großartigen RSSI sehen (z. B.`-90 dBm`) Aber Ihr Signalmesser zeigt nur **1 Bar (Schlecht)** an, Sie haben ein Problem. Das bedeutet, dass Sie lokale Störungen haben - vielleicht eine billige Stromversorgung, einen lauten Computer oder einen nahe gelegenen Funkturm - die so viel Rauschen verursachen, dass es Ihr Netz übertönt.
