...
Channel-Konversationen laden standardmäßig die neuesten **50 Nachrichten**. Scrollen du nach oben und tippen du auf **Mehr laden**, um den nächsten Stapel abzuragen. Dadurch bleibt die App auf Kanälen mit Tausenden von Nachrichten reaktionsschnell.
...
> **Tip — Kanäle teilen**
> Ein QR-Code enthält die LoRa-Konfiguration und die Kanäle, die für die Kommunikation von Funkgeräten benötigt werden. Verwenden du **Kanäle ersetzen**, um zu überschreiben, oder **Kanäle hinzufügen**, um vorhandene Kanäle hinzuzufügen.

> **Tip — Kanäle verwalten**
> Der primäre Kanal verwaltet den Rundfunkverkehr. Fügen du sekundäre Kanäle für separate Messaging-Gruppen hinzu, die jeweils durch einen eigenen Schlüssel gesichert sind.

> **Tip — Verwaltung aktiviert**
> Wählen du einen Knoten aus der Dropdown-Liste aus, um verbundene oder entfernte Geräte zu verwalten.
...
| ![PKI Mismatch](../assets/screenshots/keySlash.png) | **Public Key Mismatch** - der neueste öffentliche Schlüssel für diesen Knoten stimmt nicht mit dem zuvor aufgezeichneten Schlüssel überein. Überprüfen du, mit wem du Nachrichten senden, indem du öffentliche Schlüssel persönlich oder telefonisch vergleichen. |

---

### Tapback-Reaktionen

Drücken du lange auf eine beliebige Nachricht und tippen du auf **Tapback**, um eine Emoji-Reaktion zu senden.
...
> **Tip — Nachrichten**
> Senden du Kanalsendungen und Direktnachrichten. Drücken du lange auf eine beliebige Nachricht für Aktionen wie Kopieren, Antworten, Tapback und Lieferdetails.
...
1. **Wählen du Text aus und tippen du auf eine Schaltfläche** - Wählen du ein Wort oder einen Satz im Feld "Komponieren" aus, und tippen du dann auf eine Schaltfläche "Formatierung". Die entsprechenden Markdown-Trennzeichen werden um die Auswahl herum eingefügt. Alle vorhandenen Markdown-Trennzeichen innerhalb der Auswahl werden zuerst entfernt, um eine überlappende Syntax zu vermeiden. Leerzeichen an den Rändern der Auswahl werden außerhalb der Trennzeichen verschoben, so dass Markdown korrekt gerendert wird.
2. **Tippen du zuerst auf eine Schaltfläche und geben du dann ein** – tippen du mit dem Cursor (keine Auswahl) auf eine Formatierungsschaltfläche. Trennzeichen werden eingefügt und der Cursor wird zwischen ihnen platziert, so dass du sofort formatierten Text eingeben können.
3. **Ausschalten** - Wählen du Text aus, der bereits mit Trennzeichen umwickelt ist, und tippen du auf dieselbe Formatierungsschaltfläche, um die Trennzeichen zu entfernen.
...
Nach dem Anwenden eines Stils wird die Auswahl erweitert, um die Trennzeichen einzuschließen (z. B. Auswahl`dolphin`Und tippen du auf "Fett"`**dolphin**`), Wodurch es einfach ist, sofort auszuschalten oder zu einem anderen Stil zu wechseln.
...
Auf dem Mac Catalyst sendet das Drücken von **Enter** die Meldung. Drücken du **Umschalt+Enter**, um einen Zeilenüberbruch einzufügen. Die Schaltfläche der Zeichenpalette bleibt neben den Formatierungsschaltflächen verfügbar.
