...
Diese Seite behandelt die Implementierung der CarPlay-Funktion. Die benutzerorientierte Anleitung finden du unter [CarPlay](../user/carplay.md).
...
1. Erstellen du einen Handler in`Meshtastic/Intents/`Anpassung an die entsprechenden`INIntent`Protokoll.
2. Registrieren du den Handler in`IntentHandler.swift`'S`handler(for:)`Schalter.
3. Absichtserklärung in`Meshtastic.entitlements`Unter`com.apple.developer.siri`.
4. Fügen du eine Nutzungsbeschreibung in`Info.plist`Wenn die Absicht eine neue Datenschutzberechtigung erfordert.
