...
- Verwenden Sie nicht`XCTAssert*`In neuen Testdateien.
...
- Vergleichen Sie mit`cgImage.width`/`cgImage.height`(Pixelabmessungen im Bildschirmmaßstab), nicht`UIImage.size`(Was maßstabsabsatsabhängig ist).
- Für Ansichten mit`ScrollView`Oder keine intrinsische Höhe, bestehen Sie eine explizite`height:`Parameter zu`renderImage`.
- Verweisen Sie PNGs neben der Testdatei.
