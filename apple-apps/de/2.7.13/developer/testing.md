...
- Verwende nicht`XCTAssert*`In neuen Testdateien.

## Laufende Tests

Mit ⌘U in Xcode ausführen. Es gibt keinen CLI-Testläufer - Tests erfordern Xcode.

Stelle sicher, dass alle vorhandenen Tests bestehen, bevor du eine PR öffnen. SwiftLint wird bei jedem Commit ausgeführt; Tests, die aufgrund von Lint-Fehlern fehllaufen, blockieren CI.
...
Wenn eine Ansicht in beiden Farbschemata (z.B.`foo_light.png`+`foo_dark.png`), Einbettet beide`![]()`Tags nebeneinander führen dazu, dass beide Bilder gleichzeitig auf der Jekyll-Site und im In-App-Viewer angezeigt werden. Verwende ein HTML`<picture>`Element stattdessen:
