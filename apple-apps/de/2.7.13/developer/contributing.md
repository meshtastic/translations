...
Vielen Dank für deinen Beitrag zu Meshtastic Apple! Bitte lesen du diesen Leitfaden, bevor du eine PR eröffnen.

## Voraussetzungen

- Xcode (neueste stabile)
- IOS/macOS-Simulatoren installiert
...
Commit die regenerierten Dateien unter`Meshtastic/Resources/docs/`Mit deiner PR.
...
Verwenden du Imperativ-Stimmungs-Subjektzeilen:
...
Erklären du *was* sich im Körper geändert hat und *warum*. Halten du die Betreffzeilen unter 72 Zeichen.

## PR-Checkliste

- [ ] Alle bestehenden Tests bestehen (`⌘U`In Xcode)
- [ ] Neue Tests für neue Funktionen und Fehlerbehebungen
- [ ] SwiftLint meldet keine neuen Fehler oder Warnungen
- [ ] UI-Änderungen beinhalten Screenshots oder eine Bildschirmaufnahme in der PR-Beschreibung
- [ ] Deep-Link-Ergänzungen sind dokumentiert in`docs/developer/deep-links.md`
...
- **Nur schnell. ** Kein Ziel-C.
- **SwiftUI** für alle UI. UIKit nur dort, wo es unvermeidlich ist.
- **SF-Symbole** für alle Symbole - keine eingebetteten Bildressourcen für Symbole.
- **OSLog** für alle Protokollierungen - nein`print()`. SwiftLint setzt dies durch.
- Einzug mit **Registerkarten**.
- Öffnungsklammern an der gleichen Linie.
-`// MARK: -`Um logische Abschnitte zu trennen.
-`guard`Für einen frühen Ausgang; vermeiden du tief verschachtelte`if`.
...
- Bewachen du nur iOS-APIs:`#if !targetEnvironment(macCatalyst)`
...
3. Erstellen und verifizieren du, ob Tests bestanden werden.
4. Commit generierte Änderungen zusammen mit dem Submodul-Zeiger-Update.
