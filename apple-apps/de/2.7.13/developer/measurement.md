...
Dies sind die kanonischen Einheiten, die das Gerät sendet. Verwenden du diese immer als Quelleinheit bei der Konstruktion`Measurement`Werte:
...
> **Warning — `CLLocation.speed`Gibt m/s zurück, nicht km/h. ** Verwenden du beim Umschließen der GPS-Geschwindigkeit`UnitSpeed.metersPerSecond`. Wenn du die Quelleinheit falsch machen, führen du stillschweigend zu falschen Konvertierungen.
...
Wird verwendet, wenn du mehr Kontrolle benötigen (z. B. natürliche Skalierung für Entfernungen):
...
Verwenden du die`formattedTemperature()`Verlängerung auf`Float`(Definiert in`Meshtastic/Extensions/Float.swift`):
...
Wenn du den rohen konvertierten Wert benötigen (z. B. für Diagrammdatenpunkte), verwende`localeTemperature()`:
...
> **Warning — Erzwingen du niemals das Abwickeln von Gebietsschema-Abfragen. **`localeUnit`Kann sein`nil`Auf einigen OS-Versionen. Verwenden du immer`as? String`Mit einem sicheren Standard (Celsius).
...
Kodieren du niemals ein 12-Stunden- oder 24-Stunden-Format - lassen du das Betriebssystem es über standortbewusste Formatierer verarbeiten.
...
1. Identifizieren du die Protobuf-Quelleinheit aus dem Geräteschema
...
4. Wenn es sich um ein Diagramm handelt, stellen du sicher, dass die Achsenbeschriftungen dieselbe gebietsbewusste Konvertierung verwenden.
5. Wenn es sich um eine universelle Einheit (hPa, Grad, %) handelt, wird es so angezeigt, wie es ist
6. Testen du sowohl mit US- als auch mit metrischen Messsystemeinstellungen im Simulator
