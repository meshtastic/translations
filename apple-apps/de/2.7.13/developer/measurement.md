...
Dies sind die kanonischen Einheiten, die das Gerät sendet. Verwende diese immer als Quelleinheit bei der Konstruktion`Measurement`Werte:
...
> **Warning — `CLLocation.speed`Gibt m/s zurück, nicht km/h. ** Verwende beim Umschließen der GPS-Geschwindigkeit`UnitSpeed.metersPerSecond`. Wenn du die Quelleinheit falsch machen, führen du stillschweigend zu falschen Konvertierungen.
...
Verwende die`formattedTemperature()`Verlängerung auf`Float`(Definiert in`Meshtastic/Extensions/Float.swift`):
...
> **Warning — Erzwingen du niemals das Abwickeln von Gebietsschema-Abfragen. **`localeUnit`Kann sein`nil`Auf einigen OS-Versionen. Verwende immer`as? String`Mit einem sicheren Standard (Celsius).
...
4. Wenn es sich um ein Diagramm handelt, stelle sicher, dass die Achsenbeschriftungen dieselbe gebietsbewusste Konvertierung verwenden.
