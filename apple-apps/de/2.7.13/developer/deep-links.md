...
1. Fügen du einen Fall zu den entsprechenden`*NavigationState`Aufläumen in`Meshtastic/Router/NavigationState.swift`.
2. Auf den neuesten Stand bringen`Router`'S Routing-Helfer in`Meshtastic/Router/Router.swift`.
3. Dokumentieren du die URL in der folgenden Tabelle.
