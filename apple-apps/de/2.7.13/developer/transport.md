...
1. Füge die protobuf-Definition in der`protobufs/`Untermodul.
2. Lauf`./scripts/gen_protos.sh`.
3. Füge einen Decode/Dispatch-Fall in`AccessoryManager+FromRadio.handleFromRadio(_:)`.
4. Füge eine Send-Methode in`AccessoryManager+ToRadio.swift`.
5. Füge eine Modelleigenschaft oder eine SwiftData-Entität hinzu, wenn die Daten beibehalten werden müssen.
