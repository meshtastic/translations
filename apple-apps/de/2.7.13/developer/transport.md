...
1. Fügen du die protobuf-Definition in der`protobufs/`Untermodul.
2. Lauf`./scripts/gen_protos.sh`.
3. Fügen du einen Decode/Dispatch-Fall in`AccessoryManager+FromRadio.handleFromRadio(_:)`.
4. Fügen du eine Send-Methode in`AccessoryManager+ToRadio.swift`.
5. Fügen du eine Modelleigenschaft oder eine SwiftData-Entität hinzu, wenn die Daten beibehalten werden müssen.
6. Schreiben du Einheitstests gegen die Encode/Decod-Roundtrip.
