...
1. Fügen Sie die protobuf-Definition in der`protobufs/`Untermodul.2. Lauf`./scripts/gen_protos.sh`.3. Fügen Sie einen Decode/Dispatch-Fall in`AccessoryManager+FromRadio.handleFromRadio(_:)`.4. Fügen Sie eine Send-Methode in`AccessoryManager+ToRadio.swift`.5. Fügen Sie eine Modelleigenschaft oder eine SwiftData-Entität hinzu, wenn die Daten beibehalten werden müssen.6. Schreiben Sie Einheitstests gegen die Encode/Decod-Roundtrip.
