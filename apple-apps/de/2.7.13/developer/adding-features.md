...
Offen`Meshtastic/Router/NavigationState.swift`Und füge einen neuen Fall zu`SettingsNavigationState`:
...
## 2. Erstelle die Ansicht

Erstelle eine neue SwiftUI-Datei unter`Meshtastic/Views/Settings/`:
...
Halte die Ansichtsdateien fokussiert. Wenn die Ansicht über ~400 Zeilen hinausgeht (SwiftLint-Warnung), teilen du sie in Unteransichten oder Erweiterungsdateien auf.

## 3. Navigation "Einstellungen" verdrahten

Offen`Meshtastic/Views/Settings/Settings.swift`Und finde die`navigationDestination(for:)`Schalter. Füge deinen Fall hinzu:
...
Füge dann eine`NavigationLink`Im entsprechenden Abschnitt der Einstellungsliste:
...
Erstelle nur dann eine neue Kategorie, wenn vorhandene Kategorien nicht passen. Verfügbare Kategorien finden du im [Codebase Guide](codebase.md).

## 6. Tests schreiben

Füge eine Testdatei in`MeshtasticTests/`Mit Swift Testing:
