...
SwiftData ist die einzige Persistenzschicht.`PersistenceController.shared`Besitzt die`ModelContainer`. Ansichten verwenden`@Environment(\.modelContext)`Oder`@Query`. Hintergrund schreibt, verwenden du die`MeshPackets` `@ModelActor`.
