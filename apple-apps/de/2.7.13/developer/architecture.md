...
1. Schafft`PersistenceController.shared`(SwiftData`ModelContainer`)2. Instanzien`AppState`(Wraps`Router`)3. Instanzien`AccessoryManager`(BLE/TCP/serielle Konnektivität)4. Instanzien`AccessoryManager.shared`Als`@EnvironmentObject`Für die Ansichtshierarchie
...
| Bündel | Zweck |
|---------|---------|
| [TAKPacket-SDK](https://github.com/meshtastic/TAKPacket-SDK) | TAK V2-Drahtformat auf `ATAK_PLUGIN_V2 = Port 78`. Stellt `CotXmlParser`, `CotXmlBuilder` und `TakCompressor` (zstd Wörterbuchkomprimierung) dar. Angeheftet in `Meshtastic.xcworkspace/... /Paket.gelöst`. Siehe [TAK-Protokoll](tak-protocol.html). |
