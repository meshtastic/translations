...
Die App verwendet SwiftData ausschließlich zur Persistenz. Stelle SQLite, Realm, Core Data oder eine andere Persistenzbibliothek nicht vor.
...
Für Schreibvorgänge, die von eingehenden Funkpaketen (aus dem Hauptthread) ausgelöst werden, verwende die`MeshPackets` `@ModelActor`:
...
3. Füge eine Migrationsstufe hinzu`MeshtasticMigrationPlan.stages`:
