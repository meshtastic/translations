...
Die App verwendet SwiftData ausschließlich zur Persistenz. Stellen Sie SQLite, Realm, Core Data oder eine andere Persistenzbibliothek nicht vor.
...
| Feld | Entität | Warum |
|-------|--------|-----|
| `num` | `NodeInfoEntity` | Jedes eingehende Paket wurde nachgeschlagen |
| `num` | `UserEntity` | Ich habe jede Nachricht nachgeschlagen |
| `messageId` | `MessageEntity` | ACK-Lookups, Deduplikation |
...
Für Schreibvorgänge, die von eingehenden Funkpaketen (aus dem Hauptthread) ausgelöst werden, verwenden Sie die`MeshPackets` `@ModelActor`:
...
Schreiben Sie niemals an die wichtigsten`ModelContext`Aus einem Hintergrundthread.
...
| Art | Beschreibung |
|------|-------------|
| `NodeInfoEntity` | Ein Knoten, der auf dem Netz gehört wird |
| `MessageEntity` | Ein Kanal oder eine Direktnachricht |
| `PositionEntity` | Eine GPS-Positionsaktualisierung |
| `TelemetryEntity` | Geräte-/Umgebungssensordaten |
| `TraceRouteEntity` | Eine aufgezeichnete Spurroute |
| `WaypointEntity` | Ein gemeinsamer Kartenwegpunkt |

## Schema-Migrationen

Wenn Sie Eigenschaften auf einem`@Model`Typ, müssen Sie eine Migration bereitstellen. Schemadateien leben in`Meshtastic/Model/Schema/`.
...
3. Fügen Sie eine Migrationsstufe hinzu`MeshtasticMigrationPlan.stages`:
...
> **Warning — Löschen Sie niemals eine`VersionedSchema`. ** Der Migrationsverlauf muss beibehalten werden, da der Migrationsplan auf Geräten, die Zwischenversionen übersprungen haben, fehlschlägt.
...
Bevorzugen Sie diese Helfer gegenüber direkten Abfragen, um die Logik konsistent zu halten.
...
|-------------|-----|-----------|
| `NodeInfoEntity.positions` | 5 000 | Älteste Positionen bei Überschreitung gelöscht |
| `NodeInfoEntity.telemetries` | 5 000 pro Metriktyp | Älteste Telemetrie dieses Typs gelöscht |
| `MessageEntity`(Pro Kanal) | 50 000 | Älteste Nachrichten im Kanal gelöscht |
