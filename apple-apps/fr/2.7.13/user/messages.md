...
---
> **Conseil - Partager les canaux**> Un code QR contient la configuration LoRa et les canaux nécessaires à la communication des radios. Utilisez **Replacer les canaux** pour écraser ou **Ajouter des canaux** pour ajouter aux canaux existants.
> **Conseil - Gérer les canaux**> La chaîne principale gère le trafic de diffusion. Ajoutez des canaux secondaires pour des groupes de messagerie distincts, chacun sécurisé par sa propre clé.
> **Astuce - Administration activée**> Sélectionnez un nœud dans le menu déroulant pour gérer les appareils connectés ou distants.
---
## Messages directs
### Contacts
| Élément | Sens |
...
---
> **Astuce — Messages**> Envoyer des émissions de chaîne et des messages directs. Appuyez longuement sur n'importe quel message pour des actions telles que copier, répondre, taper et détails de livraison.
---
## Statut du message
...
---
## Apparence du lien
Les liens dans les bulles de message - y compris les URL, les liens de canal Meshtastic et les liens markdown `[text](url)` - sont stylisés avec un soulignement et les normes de conception Couleur du lien (Blue 400). Cela rend les liens visuellement distincts du texte de message ordinaire en mode clair et sombre. Appuyer sur un lien l'ouvre dans le navigateur, ou pour les URL de canal/contact Meshtastic, ouvre le gestionnaire d'application approprié.
<picture>
...
1. **Sélectionnez du texte et appuyez sur un bouton** - sélectionnez un mot ou une phrase dans le champ de composition, puis appuyez sur un bouton de formatage. Les délimiteurs de démarque appropriés sont insérés autour de la sélection. Tous les délimiteurs de démarque existants dans la sélection sont d'abord supprimés pour éviter le chevauchement de la syntaxe. L'espace blanc sur les bords de la sélection est déplacé à l'extérieur des délimiteurs afin que le marquage s'affiche correctement.2. **Appuyez d'abord sur un bouton, puis tapez** - avec le curseur placé (pas de sélection), appuyez sur un bouton de formatage. Des délimiteurs sont insérés et le curseur est placé entre eux afin que vous puissiez saisir immédiatement du texte formaté.3. **Désactiver** - sélectionnez le texte qui est déjà enveloppé de délimiteurs et appuyez sur le même bouton de formatage pour supprimer les délimiteurs.
### Aperçu en direct
...
### Styles de changement
Lorsque vous sélectionnez un texte qui contient déjà des délimiteurs de démarque et que vous appliquez un style différent, les délimiteurs existants sont supprimés et remplacés par le nouveau style. Par exemple, sélectionner `**bold**` et appuyer sur Raythrough produit `~~bold~~`.
Après avoir appliqué un style, la sélection s'élargit pour inclure les délimiteurs (par exemple, sélectionner `dauphin` et appuyer sur bold selects `**dulphin**`), ce qui facilite la désactivation ou le passage à un style différent immédiatement.
...
> **Conseil - Limite de message**> Les messages sont limités à 200 octets. Les délimiteurs Markdown comptent pour cette limite (par exemple, `**bold**` utilise 4 octets supplémentaires pour les paires `**`). Le compteur d'octets dans la barre d'outils affiche l'espace restant.
