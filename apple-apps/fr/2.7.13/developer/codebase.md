...
| Fichier | But |
|------|---------|
| `Routeur/Routeur.swift` | Contrôleur de navigation central (`@MainActor`) |
| `Routeur/État de navigation.swift` | Énumérations d'état de navigation par onglet |
| `Extensions/Logger.swift` | Enregistreurs OSLog tapés pour tous les sous-systèmes |
| `Persistance/PersistenceController.swift` | Configuration de SwiftData `ModelContainer` |
| `Modèle/MeshtasticSchema.swift` | `Schéma de version` + `Plan de migration du schéma` |
| `Accessoire/Gestionnaire d'accessoires/Gestionnaire d'accessoires.swift` | Classe racine du gestionnaire BLE/TCP |

## Modèle de fichier d'extension
Les grandes classes de gestionnaires sont divisées en fichiers `+Extension` regroupés par préoccupation :
```swift
...
Suivez le même modèle lorsque vous ajoutez de nouveaux sous-systèmes à `AccessoryManager` ou à d'autres grandes classes.
## Logging
Toute la journalisation utilise des instances `Logger` typées de `Meshtastic/Extensions/Logger.swift`. N'utilisez jamais `print()`.
```swift
...
## Voir la hiérarchie
Les vues sont dans `Meshtastic/Vues/`. Chaque fonctionnalité majeure a son propre sous-répertoire. La racine `ContentView` héberge un `TabView` tapé sur `NavigationState`.
Les vues qui ont besoin de connectivité injectent `@EnvironmentObject var bleManager : BLEManager` (nom hérité ; le nouveau code utilise `AccessoryManager`). Les vues qui ont besoin de navigation injectent `@EnvironmentObject var router : Router`.
## Localisation
