...
1. Ajoutez la définition de protobuf dans le`protobufs/`Sous-module.2. Courir`./scripts/gen_protos.sh`.3. Ajouter un dossier de décodage/expédition dans`AccessoryManager+FromRadio.handleFromRadio(_:)`.4. Ajouter une méthode d'envoi dans`AccessoryManager+ToRadio.swift`.5. Ajoutez une propriété de modèle ou une entité SwiftData si les données doivent persister.6. Écrire des tests unitaires contre l'encodage/décodage aller-retour.
