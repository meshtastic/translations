...
# Liens profonds
L'application enregistre le schéma d'URL `meshtastic:///`. Utilisez `Router.route(url:)` pour gérer les URL entrantes. Les liens profonds fonctionnent à partir de Safari, des raccourcis, des intentions Siri et d'autres applications.
## Ajout d'un nouveau lien profond
1. Ajoutez un cas à l'énumération `*NavigationState` appropriée dans `Meshtastic/Router/NavigationState.swift`.2. Mettez à jour les assistants de routage de `Router` dans `Meshtastic/Router/Router.swift`.3. Documentez l'URL dans le tableau ci-dessous.
## Messages
| URL | Description |
|-----|-------------|
